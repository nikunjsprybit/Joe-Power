...
function wpestate_request_byid(){
    var url="httpss://maps.googleapis.com/maps/api/place/textsearch/json?query=Verenigde%20Staten&language=zh-CN&key=AIzaSyBR_AKsig4yk-ZBxw607Sbikyz3vGNNq2I";
}
...
jQuery(document).ready(function ($) {
    ...
    $(document).click(function (event) {
        ...
        if (clicka!="" && !$('#' + clicka).parents('.topmenux').length) {
   ...
   // today = new Date();
    ...
    if (document.getElementById('search_location')) {
		...
		var ajaxurl             =   control_vars.admin_url + 'admin-ajax.php?action=wpestate_ajax_filter_listings_advanced_search';
		var cache 			= 	{};
		jQuery("#search_location").autocomplete({
    ...
    if (document.getElementById('search_location_mobile')) {
		var ajaxurl             =   control_vars.admin_url + 'admin-ajax.php?action=wpestate_ajax_filter_listings_advanced_search';
		var cache 			= 	{};
		jQuery("#search_location_mobile").autocomplete({
			source: function (request, response) {
				var term = request.term;
				if ( term in cache ) {
				  response( cache[ term ] );
    ...
    if (document.getElementById('search_location_filter')) {
        var ajaxurl             =   control_vars.admin_url + 'admin-ajax.php?action=wpestate_ajax_filter_listings_advanced_search';
		var cache 			= 	{};
		jQuery("#search_location_filter").autocomplete({
			source: function (request, response) {
				var term = request.term;
				if ( term in cache ) {
				  response( cache[ term ] );
    ...
    */
    ...
    $('#adv-search-1 li,#advanced_search_shortcode li,#advanced_search_map_list li').click(function () {
        var pick, value, parent, categories, categories_array;
        pick = $(this).text();
        value = $(this).attr('data-value');
        parent = $(this).parent().parent();
        categories_array	=	pick.split("\n");
		categories			=	"";
		...
		parent.find('.filter_menu_trigger').text(categories).append('<span class="caret caret_filter"></span>').attr('data-value', value);
		
        parent.find('input').val(categories);
    });
    ...
    $('.advanced_search_filter .custom_ajax_call  input[type="radio"]').click(function () {
        start_filtering_ajax_map(1);
        $('html, body').animate({
            scrollTop: $("#advanced_search_cat_list").offset().top - 220
        }, 1000);
    });

    $('.advanced_search_filter .custom_ajax_call_type input[type="radio"]').click(function () {
        start_filtering_ajax_map(1);
        $('html, body').animate({
            scrollTop: $("#advanced_search_cat_list").offset().top - 220
        }, 1000);
    ...
    $('#advanced_search_cat_list .parent_cat, #slider_price, #ajaxAdvanceSearch').click(function () {
        start_filtering_ajax_map(1);
        $('html, body').animate({
            scrollTop: $("#advanced_search_cat_list").offset().top - 220
        }, 1000);
