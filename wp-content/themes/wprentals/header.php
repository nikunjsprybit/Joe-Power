...
<html <?php language_attributes(); ?>>
    ...
    if( is_page() && is_user_logged_in()  && (is_page_template('vendor_dashboard.php') || wpestate_check_if_admin_page($post->ID)) ){
        $is_dashboard_page='is_dashboard_page';
    } else if ( is_page_template('user_dashboard_add_step1.php') ) {
        $is_dashboard_page='is_dashboard_page';
    }
...
<?php
if (!is_search() && !is_tag() && !is_404() && !is_tax() && !is_category() && ( basename(get_page_template($post->ID)) === 'property_list_half.php' || get_post_type() === 'estate_property' || get_post_type() === 'product' )) {
    //do nothing for now  
} else if (( is_category()) && get_option('wp_estate_property_list_type', '') ==  2 ) {
    if( !is_tax() ){
    ...
    //do nothing for now 
} else if (is_single('product') || is_page_template('template-nomap.php') || is_page_template('contact_page.php')){
    // get_template_part('header_media');
}
else {
	...
	if ( $taxonmy !=='property_action_category' && $taxonmy!='property_category' && esc_html(get_option('wp_estate_use_upload_tax_page',''))==='yes' ){
			if($taxonmy=="dc_vendor_shop"){
				// do not show photo header for vendor profile
			}else{
				get_template_part('header_media');
			}
	}else{
...
if ($map_template === 1) {
	...
	if (!is_404() && !is_category() && !is_search() && !is_tag()) {
        if ( wpestate_check_if_admin_page($post->ID) || is_page_template('vendor_dashboard.php')) {
