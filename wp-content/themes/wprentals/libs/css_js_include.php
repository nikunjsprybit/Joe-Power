...
function wpestate_scripts() {   
    ...
    if( !is_tax() && (get_post_type() === 'estate_property' || get_post_type() === 'product') ) {
        wp_enqueue_script('wpestate_jquery.fancybox.pack', trailingslashit( get_template_directory_uri() ).'js/jquery.fancybox.pack.js',array('jquery'), '1.0', true); 
        wp_enqueue_script('wpestate_jquery.fancybox-thumbs', trailingslashit( get_template_directory_uri() ).'js/jquery.fancybox-thumbs.js',array('jquery'), '1.0', true); 
    ...
    if (esc_html ( get_option('wp_estate_ssl_map','') ) =='yes'){
        wp_enqueue_script('wpestate_googlemap', 'https://maps.googleapis.com/maps/api/js?libraries=places&language=en&key='.esc_html(get_option('wp_estate_api_key', '') ),array('jquery'), '1.0', false);        
    }else{
        wp_enqueue_script('wpestate_googlemap', 'https://maps.googleapis.com/maps/api/js?libraries=places&language=en&key='.esc_html(get_option('wp_estate_api_key', '') ),array('jquery'), '1.0', false);        
    }
    ...
    if( (get_post_type() === 'estate_property' || get_post_type() === 'product') && !is_tax() && !is_search() && !is_tag() ){
        $load_extra =   1;
...
//    $login_redirect =   wpestate_get_dasboard_add_listing();
    ...
    wp_localize_script('wpestate_ajaxcalls', 'ajaxcalls_vars', 
            array(  'contact_name'          =>  esc_html__( 'Your Name','wpestate'),
                    ...
                    'login_loading'         =>  esc_html__( 'Sending user info, please wait...','wpestate'), 
                    'userid'                =>  $userID,
                    'prop_featured'         =>  esc_html__( 'Equipment is featured','wpestate'),
                    'no_prop_featured'      =>  esc_html__( 'You have used all the "Featured" listings in your package.','wpestate'),
                    'favorite'              =>  esc_html__( 'Favorite','wpestate').'<i class="fa fa-star"></i>',
    ...
    if(is_page_template('user_dashboard_allinone.php') || is_page_template('user_dashboard_edit_listing.php') || is_page_template('user_dashboard_add_step1.php') ||  ( 'estate_property' == get_post_type() || 'product' == get_post_type() )  ){
    ...
    if( 'estate_property' == get_post_type() ||  'product' == get_post_type() ||  'estate_agent' == get_post_type() ){
    ...
    if( (get_post_type() === 'estate_property' || get_post_type() === 'product')&& !is_tax() ){
        wp_enqueue_script('wpestate_property',trailingslashit( get_template_directory_uri() ).'js/property.js',array('jquery'), '1.0', true); 
    }
...
function wpestate_admin($hook_suffix) {	
    ...
    if (is_admin() &&  ( $pagenow=='post-new.php' || $pagenow=='post.php') && ($typenow=='estate_property' || $typenow=='product')) {
        if (esc_html ( get_option('wp_estate_ssl_map','') ) =='yes'){
            ...
            wp_enqueue_script('wpestate_googlemap',      'https://maps.googleapis.com/maps/api/js?key='.esc_html(get_option('wp_estate_api_key', '') ).'&amp;sensor=true',array('jquery'), '1.0', false);
        }  
