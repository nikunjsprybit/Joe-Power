<?php
add_action('wp_ajax_nopriv_load_calander_events_ajax', 'load_calander_events_ajax');
add_action('wp_ajax_load_calander_events_ajax', 'load_calander_events_ajax');

function load_calander_events_ajax() {
    ...
    if (!empty($products)) {
        foreach ($products as $product) {
            $vendor_products[] = $product->ID;
        }
    }
    $bookings = WC_Bookings_Controller::get_bookings_in_date_range(
                    strtotime($_REQUEST['start']), strtotime($_REQUEST['end']), $vendor_products, false
    );
    $calander_data = array();
    if (!empty($bookings)) {
        foreach ($bookings as $booking) {
            ...
            $calander_data[] = array(
                'id' => $booking->ID,
                'title' => $product_title,
                'start' => date("Y-m-d h:i:s", $booking->get_start()),
                'end' => date("Y-m-d h:i:s", $booking->get_end())
            );
        }
    }
    echo json_encode($calander_data);
    wp_die();
}
    ...
    function wpestate_update_menu_bar() {
        ...
        if ($user_id != 0 && $user_id != '') {
            ...
            if ($home_url != $dash_link) {
                $menu .= ' <a href="' . $dash_link . '" ><i class="fa fa-map-marker"></i>' . esc_html__('My Equipment List', 'wpestate') . '</a>';
            }

            if ($home_url != $add_link) {
                $menu .= '<a href="' . $add_link . '" ><i class="fa fa-plus"></i>' . esc_html__('Add New Equipment', 'wpestate') . '</a>';
            }

            if ($home_url != $dash_favorite) {
                $menu .= '<a href="' . $dash_favorite . '" class="active_fav"><i class="fa fa-star"></i>' . esc_html__('Favorites', 'wpestate') . '</a>';
            }
...
if (!function_exists('wpestate_ajax_update_profile')):
    ...
    function wpestate_ajax_update_profile() {
        ...
        $hobbies = sanitize_text_field(wp_kses($_POST['hobbies'], $allowed_html));
        ...
        update_user_meta($userID, 'hobbies', $hobbies);
        ...
        wpestate_update_user_agent($agent_id, $firstname, $secondname, $useremail, $userphone, $userskype, $profile_image_url, $usermobile, $about_me, $hobbies, $profile_image_url_small, $userfacebook, $usertwitter, $userlinkedin, $userpinterest, $live_in, $i_speak, $login_name, $payment_info);
...
if (!function_exists('wpestate_update_user_agent')):
    ...
    function wpestate_update_user_agent($agent_id, $firstname, $secondname, $useremail, $userphone, $userskype, $profile_image_url, $usermobile, $about_me, $hobbies, $profile_image_url_small, $userfacebook, $usertwitter, $userlinkedin, $userpinterest, $live_in, $i_speak, $login_name, $payment_info) {
    ...
    function wpestate_ajax_show_contact_owner_form() {
        ...
        if (is_user_logged_in()) {
            ...
            <div class="modal-header"> 
                ...
                <h2 class="modal-title_big"><?php echo esc_html__('Contact the owner', 'wpestate'); ?></h2>
                <h4 class="modal-title" id="myModalLabel">
            <?php echo esc_html__('Please complete the form below to contact owner.', 'wpestate'); ?>
                </h4>
            </div>
            <div class="modal-body">
                <div id="booking_form_request_mess_modal"></div>  
                <!-- <div class=" has_calendar calendar_icon">
                    <input type="text" id="booking_from_date" size="40" name="booking_from_date" class="form-control" placeholder="<?php echo esc_html__('Check In', 'wpestate'); ?>" value="">
                </div>

                <div class=" has_calendar calendar_icon">
                    <input type="text" id="booking_to_date" size="40" name="booking_to_date" class="form-control" placeholder="<?php echo esc_html__('Check Out', 'wpestate'); ?>" value="">
                </div> -->

                <!-- <div class="">
                    <select id="booking_guest_no"  name="booking_guest_no"  class="cd-select form-control" >
                        <option value="1">1 <?php echo esc_html__('Guest', 'wpestate'); ?></option>'
                        for ($i = 2; $i <= 14; $i++) {
                            print '<option value="<?php echo $i; ?>"><?php echo $i; ?> <?php echo esc_html__('Guests', 'wpestate'); ?></option>';
                        }
                    print'
                    </select>    
                </div> -->

                <input type="hidden" id="property_id" name="property_id" value="<?php echo $post_id; ?>" />
                <input name="prop_id" type="hidden"  id="agent_property_id" value="<?php echo $post_id; ?>">
                <input name="agent_id" type="hidden"  id="agent_id" value="<?php echo $agent_id; ?>">

                <div class="">
                    <textarea id="booking_mes_mess" name="booking_mes_mess" cols="50" rows="6" placeholder="<?php echo esc_html__('Your message', 'wpestate'); ?>" class="form-control"></textarea>
                </div>

                <button type="submit" id="submit_mess_front" class="wpb_button  wpb_btn-info  wpb_regularsize   wpestate_vc_button  vc_button"><?php echo esc_html__('Send Message', 'wpestate'); ?></button>
    ...
    function wpestate_ajax_show_login_form() {
                ...
                <div class="modal fade" id="loginmodal" tabindex="-1" aria-labelledby="myModalLabel" aria-hidden="true">
                  <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                      <div class="modal-header"> 
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                        <h2 class="modal-title_big" >' . esc_html__('Start Listing Equipment', 'wpestate') . '</h4>
                        ...
                        <div id="ajax_login_div" class="' . $show_login . '">
                            ...
                            <div class="login_form" id="login-div">
                                ...
                                <button id="wp-login-but" class="wpb_button  wpb_btn-info  wpb_regularsize   wpestate_vc_button  vc_button" data-mixval="' . $propid . '">' . esc_html__('Login', 'wpestate') . '</button>
                                    ...
                                    <div class="navigation_links cstm_frgtpass_acccreate_btn">
        ...
        print'
                            ...
                            <div id="ajax_register_div" class="' . $show_register . '">
                                ...
                                <div id="reveal_login" class="cstm_member_text"><a href="#">' . esc_html__('Already a member? Sign in!', 'wpestate') . '</div> ';
...
if (!function_exists('wpestate_get_filtering_ajax_result')):
    ...
    function wpestate_get_filtering_ajax_result() {
        ...
        $currency = esc_html(get_option('wp_estate_currency_label_main', ''));
        $where_currency = esc_html(get_option('wp_estate_where_currency_symbol', ''));
        $area_array = $city_array = $action_array = '';
        $categ_array = '';
...
if (!function_exists('wpestate_ajax_google_login_oauth')):
    ...
    function wpestate_ajax_google_login_oauth() {
        ...
        $google_redirect_url = wpestate_get_dashboard_profile_link('google');
...
if (!function_exists('wpestate_ajax_facebook_login')):
    ...
    function wpestate_ajax_facebook_login() {
        ...
        print $loginUrl = $helper->getLoginUrl(wpestate_get_dashboard_profile_link('facebook'), $permissions);
...
add_action('wp_ajax_wpestate_ajax_filter_listings_advanced_search', 'wpestate_ajax_filter_listings_advanced_search');
add_action('wp_ajax_nopriv_wpestate_ajax_filter_listings_advanced_search', 'wpestate_ajax_filter_listings_advanced_search');

if (!function_exists('wpestate_ajax_filter_listings_advanced_search')):

    function wpestate_ajax_filter_listings_advanced_search() {

        $search_keyword = sanitize_text_field($_REQUEST['query']);
        ...
        $equipements = array();
        while ($prop_selection->have_posts()): $prop_selection->the_post();
            $equipements[] = array('label' => get_the_title(), 'value' => get_the_title(), 'url' => esc_url(get_permalink()));
        endwhile;
