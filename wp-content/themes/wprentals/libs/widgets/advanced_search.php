	...
	function widget($args, $instance){
		...
		if($title) {
                ...
                if($show_adv_search_general=='no'){
                    ...
                    <input type="text" id="ssearch_location_filter_widget<?php echo $wpestate_internal_search; ?>" class="form-control search_location_city" name="search_location" placeholder="<?php echo esc_html__( 'Search Equipment','wpestate'); ?>" value="" autocomplete="off">
                    ...
                    <select name="select_category" id="select_category" class="dropdown form-control">
                        <option>All</option>
                        <?php 
                        $product_categories   = get_terms( 'product_cat', 'orderby=name&hide_empty=true&parent=0' );
                        if ( $product_categories ) {
                            foreach ( $product_categories as $cat ) {
                                echo '<option value="' . esc_attr( $cat->name ) . '">' . esc_html( $cat->name ) . '</option>';
                                $product_child_categories   = get_terms( 'product_cat', 'orderby=name&hide_empty=true&parent=' . absint( $cat->term_id ) );
                                if ( $product_child_categories ) {
                                    foreach ( $product_child_categories as $child_cat ) {
                                        echo '<option value="' . esc_attr( $child_cat->name ) . '">' . '&nbsp;&nbsp;' . esc_html( $child_cat->name ) . '</option>';
                                    }
                                }
                            }
                        }
