    ...
    function estate_sidebar_meta() {
        ...
        if(isset($post->ID)){
            if(  basename(get_page_template($post->ID))== 'property_list.php' || basename(get_page_template($post->ID))== 'property_list_half.php' ){
                add_meta_box('wpestate-pro_list_adv',       esc_html__( 'Equipment List Advanced Options','wpestate'), 'estate_prop_advanced_function', 'page', 'normal', 'low');
            }
        }
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////
///  Equipment Listing advanced options
///////////////////////////////////////////////////////////////////////////////////////////////////////////
...
function estate_prop_advanced_function(){
    ...
    if( basename(get_page_template($post->ID))!= 'property_list.php' && basename(get_page_template($post->ID))!= 'property_list_half.php' ){
        esc_html_e('Only for "Equipments List" page template ! ','wpestate');
        return;
    }
    ...
    function estate_listing_options(){
        ...
        <p class="meta-options">
         ...
         ';
            ...
            print esc_html_e('These Options are available for "Equipment list" page template only!','wpestate');
        }
...
if( !function_exists('estate_page_video_box') ):
    function estate_page_video_box($post) {
        ...
        print '  
        <p class="meta-options pblank">
            <h3 class="pblankh">'.esc_html__( 'Options for video','wpestate').'</h3>
        </p>
        <p class="meta-options">	
            <label for="page_custom_lat">'.esc_html__( 'Video Url','wpestate').'</label><br />
            <input type="text" id="video_url" name="video_url" size="40" value="'.$video_url.'">
        </p>
        ';
    }
endif; // end   estate_page_video_box  
