...
$category       =   get_the_term_list($postid, 'product_cat', '', ', ', '');
if ( $category == '' ){
    echo $category=get_the_category_list(', ',$postid);
}
    ...
    if (is_archive()) {
		if( is_category() || is_tax() ){
			...
			if($category->parent!=0){
				...
				$parent_category  = get_term_by( 'id', $category->parent, 'product_cat', 'ARRAY_A' );
				print '<li class=""><a href="'.get_term_link($parent_category['term_id']).'">'. $parent_category['name'] .'</a></li>';
			}
