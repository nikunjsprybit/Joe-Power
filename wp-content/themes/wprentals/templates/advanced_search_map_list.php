    ...
    <div class="advanced_search_map_list_container <?php echo $advanced_search_filter; ?>">
        ...
        <?php } else { ?>  
            <div class="<?php echo $col_class; ?> map_icon">
                ...
                ?>
                <input type="text"   id="search_location_filter<?php echo $wpestate_internal_search; ?>" class="form-control search_location_city" name="search_location" placeholder="<?php esc_html_e('Search Equipment', 'wpestate'); ?>" value="<?php
                if (isset($_GET['search_location'])) {
                    echo wp_kses(esc_attr($_GET['search_location']), $allowed_html);
                }
                ?>" >              
                <input type="hidden" id="search_location_city" value="<?php
                if (isset($_GET['advanced_city'])) {
                    echo wp_kses(esc_attr($_GET['advanced_city']), $allowed_html);
                }
                ?>" >
                <input type="hidden" id="search_location_area" value="<?php
                       if (isset($_GET['advanced_area'])) {
                           echo wp_kses(esc_attr($_GET['advanced_area']), $allowed_html);
                       }
                       ?>" >
                <input type="hidden" id="search_location_country"    value="<?php
            if (isset($_GET['advanced_country'])) {
                echo wp_kses(esc_attr($_GET['advanced_country']), $allowed_html);
            }
            ?>" >              
                <input type="hidden" id="property_admin_area" name="property_admin_area"  value="<?php
                        if (isset($_GET['property_admin_area'])) {
                            echo wp_kses(esc_attr($_GET['property_admin_area']), $allowed_html);
                        }
                        ?>" >
            </div>
            <div class="<?php echo $col_class; ?>">
