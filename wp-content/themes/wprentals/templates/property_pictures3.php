...
if ( !empty($attachment_ids) ) {
   array_splice($attachment_ids, array_search(get_post_thumbnail_id(), $attachment_ids ), 1);
}
...
if (function_exists('icl_translate') ){
    ...
    $prop_stat = apply_filters( 'wpml_translate_single_string', $prop_stat, 'wpestate', 'property_status_'.$prop_stat );
}

$ribbon_class       = str_replace(' ', '-', $prop_stat);    
        
$total_pictures=count ($attachment_ids)+1;      
//print '<div class="panel-title panel_pictures">'.esc_html__( 'Equipment Pictures').'<span class="pictures_explanation">  '. $total_pictures .' '.esc_html__( 'photos','wpestate').'</span></div>';


if ($attachment_ids || has_post_thumbnail() || get_post_meta($post->ID, 'embed_video_id', true)) {  ?>   
    <div id="carousel-listing" class="carousel slide post-carusel carouselvertical" data-ride="carousel" data-interval="false"> <?php  
        $indicators='';
        $round_indicators='';
        $slides ='';
        $captions='';
        ...
        if($video_id!=''){
            ...
            $indicators.='<li data-target="#carousel-listing"  
                data-video_data="'.$video_type.'" data-video_id="'.$video_id.'"  data-slide-to="0" class="active video_thumb_force">
        ...
        if( has_post_thumbnail() ){
            ...
            $slides .= '<div class="item '.$active.' ">
                <a href="'.$full_img[0].'" title="'.$attachment_meta['caption'].'"  rel="data-fancybox-thumb" class="fancybox-thumb">
                <img  src="'.$full_img[0].'" data-original="'.$full_prty[0].'" alt="'.$attachment_meta['alt'].'" class="img-responsive" />';
                $slides .= '</a>';
                if($attachment_meta['caption']!=''){
                    $slides .= '<div class="item_captions_text">'.$attachment_meta['caption'].'</div><div class="item_captions"></div>';
                }
            $slides .=  '</div>';
        }
        ...
        foreach ($attachment_ids as $attachment_id) {
            ...
            if ($slider_size=='full'){
                $full_img           = wp_get_attachment_image_src($attachment_id, 'listing_full_slider_1');
            }else{
                $full_img           = wp_get_attachment_image_src($attachment_id, 'listing_full_slider');
            }
            ...
            $slides .= '<div class="item '.$active.'">
                <a href="'.$full_img[0].'" rel="data-fancybox-thumb" title="'.$attachment_meta['caption'].'" class="fancybox-thumb">
                    <img  src="'.$full_img[0].'" data-original="'.$full_prty[0].'" alt="'.$attachment_meta['alt'].'" class="img-responsive" />';
                    if($attachment_meta['caption']!=''){
                        $slides .= '<div class="item_captions_text">'.$attachment_meta['caption'].'</div><div class="item_captions"></div>';
                    }
