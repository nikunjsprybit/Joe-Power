...
$name = ( $user_login->display_name )? $user_login->display_name : $user_login->user_nicename;

$name = ($name) ? $name : $user_login;

$user_custom_picture    =   get_the_author_meta( 'custom_picture' , $post_author );
if($user_custom_picture==''){
    $user_custom_picture    =   get_template_directory_uri().'/img/default_user_agent.gif';
}
$link = site_url().'/vendor/'.$user_login;
?>
...
<div class="agentpic-wrapper">
    <?php if($content!=''){      print $content;   } ?>
     <div  id="contact_me_long" class="  " data-postid="2275" ><?php esc_html_e('Contact Owner','wpestate');?></div>
</div>