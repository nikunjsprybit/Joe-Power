...
class WC_Email_Vendor_New_Product_Added extends WC_Email {
	...
	function trigger( $post_id, $post, $vendor )	{
		...
		if($post_type == 'shop_coupon') {
			...
			$this->heading = __( 'New coupon submitted: {product_name}', 'dc-woocommerce-multi-vendor' );
			$this->subject = __( '[{blogname}] New coupon submitted by {vendor_name} - {product_name}', 'dc-woocommerce-multi-vendor' );
		}
