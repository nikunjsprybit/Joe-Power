...
class WC_Bookings_Ajax {
	...
	public function calculate_costs() {
		...
		if ( isset($posted['selete_rent_type']) && empty($posted['selete_rent_type']) ) {
			wp_send_json( array(
				'result' => 'ERROR',
				'html'   => '<span class="booking-error">Rental period is required - please select one above</span>',
			) );
		}
		...
		if ( $posted['selete_rent_type'] == 'Day(s)' ) {
			$cost = $wc_booking_day_cost;
		}
		if ( $posted['selete_rent_type'] == 'Week(s)' ) {
			$cost = $wc_booking_week_cost;
		}
		else if ( $posted['selete_rent_type'] == 'Month(s)' ) {
			$cost = $wc_booking_month_cost;
		}
