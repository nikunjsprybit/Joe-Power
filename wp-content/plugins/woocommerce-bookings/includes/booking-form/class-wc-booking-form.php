...
class WC_Booking_Form {
	...
	public function get_posted_data( $posted = array() ) {
		...
		$start_date 	=	explode("-", $posted['start_date']);
		$posted['wc_bookings_field_start_date_year']	=	$start_date[0];
		$posted['wc_bookings_field_start_date_month']	=	$start_date[1];
		$posted['wc_bookings_field_start_date_day']		=	$start_date[2];
		...
		// Duration
		if ( 'customer' == $this->product->get_duration_type() ) {
			$booking_duration       = 	(strtotime($posted['end_date'])- strtotime($posted['start_date']))/86400;
	...
	public function calculate_booking_cost( $posted ) {
		...
		if($data['_duration']%7==0){
			$this->booking_cost 	=	($data['_duration']/7) * $wc_booking_week_cost;
		}else if($data['_duration']%30==0){
			$this->booking_cost 	=	($data['_duration']/30) * $wc_booking_month_cost;
		}else{
			$this->booking_cost 	=	($data['_duration']) * $wc_booking_day_cost;
		}
