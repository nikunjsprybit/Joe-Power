...
class WCFM_Fields {
  ...
  public function upload_input($field) {
    ...
    $gallery_img_id 		= isset( $field['gallery_img_id'] ) ? $field['gallery_img_id'] : '';

    $product_image_gallery = array();
    if ( isset($field['product_image_gallery']) && !empty($field['product_image_gallery']) ) {
      $galleryIdArr = explode(',', $field['product_image_gallery']);

      if ( !empty($field['gallery_img_id']) && isset($field['gallery_img_id']) ) {
        array_splice($galleryIdArr, array_search($field['gallery_img_id'], $galleryIdArr ), 1);
      }
    }

    if($field['mime'] == 'image') {
      $mimeProp = '<div class="uploaded_images imageid_'.$gallery_img_id.'" data-imageid="'.$gallery_img_id.'"><input type="hidden" value="'.$gallery_img_id.'" name="product_image_gallery[]"><img id="'.esc_attr($field['id']).'_display" src="'.esc_attr( $field['value'] ).'" height="'.absint( $field['prwidth'] ).'" class="'.$placeHolder.'" /><i class="fa deleter fa-trash-o '.esc_attr($field['id']).'" data-imageid="'.$gallery_img_id.'"></i><i class="fa thumber fa-star set_featured"></i></div>';
    } else {
      if($field['value'])
    ...
    echo '<div id="imagelist" class="ui-sortable">';
    printf('%s',  $mimeProp );

    // print_r($galleryIdArr);

    if ( count($galleryIdArr) > 0 ) {
      foreach ($galleryIdArr as $key => $value) { 
        $gallery_img_urls = wp_get_attachment_url($value);
        ?>
        <div class="uploaded_images imageid_<?php echo $value; ?>" data-imageid="<?php echo $value; ?>">
          <input type="hidden" value="<?php echo $value; ?>" name="product_image_gallery[]">
          <img src="<?php echo $gallery_img_urls; ?>" height="100px" alt="thumb"><i class="fa deleter fa-trash-o" data-imageid="<?php echo $value; ?>"></i>
        </div>
        <?php
      }
    }

    $uploadicon = get_stylesheet_directory_uri().'/img/upload-icon.jpg';

    printf(
        '
        <span class="wcfm-wp-fields-uploader %s">
          <input type="text" name="%s" id="%s" style="%s" class="%s" readonly value="%s" %s data-mime="%s" />
          <input type="text" value="%s" name="product_feimage" id="product_feimage" style="display:none;">
          <img src="%s" class="upload_button button button-secondary" name="%s_button" id="%s_button" data-mime="%s" aria-hidden="true" style="height: 100px; margin-bottom: 20px;"></img>
    ...
    echo '</div><p class="full_form full_form_image"> *Double Click on the image to select featured. <br> **Change images order with Drag &amp; Drop. <br> *Maximum upload file size: 5 MB. <br>* Allow only jpg, jpeg, png and gif format files.</p>';
    $this->field_wrapper_end($field);
  }
  ...
  public function field_wrapper_start($field) {
    ...
    if(isset($field['label'])) {
      ...
      if(isset($field['hints'])) {
        printf(
          '<a href="javascript:void(0);" data-toggle="tooltip" data-placement="top" class="img_tip" title="%s"></a>',
  ...
  public function field_wrapper_end($field) {
    ...
    if(!isset($field['label']) && isset($field['hints'])) {
      ...
      printf(
        '<span class="img_tip" data-tip="%s"></span>',
        wp_kses_post ( $field['hints'] )
      );
