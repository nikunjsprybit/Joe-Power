...
class WCFM_WCVendors {
  ...
  function wcvendors_wcfm_menus( $menus ) {
  	...
  	if( !$can_view_orders ) unset( $menus['orders'] );
  	if( !$can_view_sales ) unset( $menus['reports'] );
  ...
  function wcvendors_after_line_total_head( $order ) {
  	global $WCFM;
  	?>
  	<th class="line_cost sortable no_mob" data-sort="float"><?php _e( 'Rate', $WCFM->text_domain ); ?></th>
  ...
  function wcvendors_after_line_total( $item, $order ) {
  	...
  	?>
  	<td class="line_cost no_mob" width="1%">
		  <div class="view"><?php echo sprintf( '%.2f%%', $commission_rate ); ?></div>
		</td>
		<?php
		...
		?>
			...
			<td class="line_cost no_ipad no_mob" width="1%">
				<div class="view"><?php echo wc_price( $order_line_due[0]->total_shipping ); ?></div>
			</td>
			<?php } ?>
			<?php if ( WC_Vendors::$pv_options->get_option( 'give_tax' ) ) { ?>
			<td class="line_cost no_ipad no_mob" width="1%">
				<div class="view"><?php echo wc_price( $order_line_due[0]->tax ); ?></div>
			</td>
			<?php } ?>
		<?php
		} else {
    ...
    $order_due = $wpdb->get_results( $sql );
  	$sum = WCV_Queries::sum_for_orders( array( $order_id ), array('vendor_id'=>get_current_user_id()) ); 
		...
		<tr>
			...
			<td class="total">
				<div class="view"><?php echo wc_price( $order_due[0]->shipping ); ?></div>
			</td>
		</tr>
		<?php } ?>
		<?php if ( WC_Vendors::$pv_options->get_option( 'give_tax' ) ) { ?>
  ...
  function wcvendors_reports_get_order_report_data( $result ) {
  	...
  	$products = array();
		foreach ($vendor_products as $_product) {
			...
			if( $_product->post_type == 'product_variation' ) $products[] = $_product->post_parent;
		}
