...
if ( ! is_object( $theorder ) ) {
	if( isset( $wp->query_vars['wcfm-orders-details'] ) && !empty( $wp->query_vars['wcfm-orders-details'] ) ) {
		$theorder = wc_get_order( $wp->query_vars['wcfm-orders-details'] );
	}
}
...
if( $is_wcfm_order_details_tax_line_item = apply_filters( 'wcfm_order_details_tax_line_item', true ) ) {
	if ( wc_tax_enabled() ) {
		...
		$classes_options[''] = __( 'Standard', $WCFM->text_domain );
...
<div class="collapse wcfm-collapse" id="wcfm_order_details">
	...
	<!-- collapsible -->
	<div class="page_collapsible orders_details_general" id="wcfm_general_options">
	  <?php _e('Order Details', $WCFM->text_domain); ?><span></span>
	  <?php
		if( $allow_wp_admin_view = apply_filters( 'wcfm_allow_wp_admin_view', true ) ) {
			?>
			<a target="_blank" class="wcfm_wp_admin_view text_tip" href="<?php echo admin_url('post.php?post='.$order_id.'&action=edit'); ?>" data-tip="<?php _e( 'WP Admin View', $WCFM->text_domain ); ?>"><span class="wcicon-user2"></span></a>
			<?php
		}
		?>
	</div>
	<div class="wcfm-container">
		<div id="orders_details_general_expander" class="wcfm-content">
			...
			<p class="form-field form-field-wide"><label for="order_date"><?php _e( 'Order date:', $WCFM->text_domain ) ?></label>
			...
			<?php if( $is_allow_order_status_details = apply_filters( 'wcfm_allow_order_details', true ) ) { ?>
				<p class="form-field form-field-wide wc-order-status"><label for="order_status"><?php _e( 'Order status:', $WCFM->text_domain ) ?> <?php
						if ( $order->needs_payment() ) {
							printf( '<a target="_blank" href="%s">%s &rarr;</a>',
								esc_url( $order->get_checkout_payment_url() ),
								__( 'Customer payment page', $WCFM->text_domain )
							);
						}
					?></label>
					<?php
						...
						foreach ( $statuses as $status => $status_name ) {
							if( $status == 'wc-' . $order->get_status() ) echo apply_filters( 'wcfm_order_status_modify', '<span class="order-status wcicon-status-' . sanitize_title( $order->get_status() ) . '"></span>&nbsp;' . esc_html( $status_name ), $order );
						}
					?>
				</p>
	
				<p class="form-field form-field-wide wc-customer-user">
					<label for="customer_user"><?php _e( 'Customer:', $WCFM->text_domain ) ?> <?php
						if ( $order->get_user_id() ) {
							...
							printf( '<a target="_blank" href="%s">%s &rarr;</a>',
								...
								__( 'View other orders', $WCFM->text_domain )
							);
						}
					?></label>
					<?php
				...
				<p class="order_number">
				  <?php
						...
						if ( $payment_method ) {
							printf( __( '<label for="order_payment_via">Payment via: </label> %s', $WCFM->text_domain ), ( isset( $payment_gateways[ $payment_method ] ) ? esc_html( $payment_gateways[ $payment_method ]->get_title() ) : esc_html( $payment_method ) ) );
						...
						if ( $ip_address = $order->get_customer_ip_address() ) {
							echo __( 'Customer IP', $WCFM->text_domain ) . ': <span class="woocommerce-Order-customerIP">' . esc_html( $ip_address ) . '</span>';
						}
				  ?>
				</p>
			<?php } ?>
			...
			<table>
				<thead>
					<tr>
						<th>
							<?php _e( 'Billing Details', $WCFM->text_domain ); ?>
						</th>
						<th>
							<?php _e( 'Shipping Details', $WCFM->text_domain ); ?>
						</th>
					</tr>
				</thead>
				<tbody>
						...
						<td>
							<?php
								...
								if ( $order->get_formatted_billing_address() ) {
									echo '<p><strong>' . __( 'Address', $WCFM->text_domain ) . ':</strong>' . wp_kses( $order->get_formatted_billing_address(), array( 'br' => array() ) ) . '</p>';
								} else {
									echo '<p class="none_set"><strong>' . __( 'Address', $WCFM->text_domain ) . ':</strong> ' . __( 'No billing address set.', $WCFM->text_domain ) . '</p>';
								}
							...
							<?php
								...
								echo '<div class="address">';
									...
									if ( $order->get_formatted_shipping_address() ) {
										echo '<p><strong>' . __( 'Address', $WCFM->text_domain ) . ':</strong>' . wp_kses( $order->get_formatted_shipping_address(), array( 'br' => array() ) ) . '</p>';
									} else {
										echo '<p class="none_set"><strong>' . __( 'Address', $WCFM->text_domain ) . ':</strong> ' . __( 'No shipping address set.', $WCFM->text_domain ) . '</p>';
									}
									...
									if ( apply_filters( 'woocommerce_enable_order_notes_field', 'yes' == get_option( 'woocommerce_enable_order_comments', 'yes' ) ) && $post->post_excerpt ) {
										echo '<p><strong>' . __( 'Customer Provided Note', $WCFM->text_domain ) . ':</strong> ' . nl2br( esc_html( $post->post_excerpt ) ) . '</p>';
									}
	...
	<!-- collapsible -->
	<div class="page_collapsible orders_details_items" id="wcfm_orders_items_options"><?php _e('Order Items', $WCFM->text_domain); ?><span></span></div>
	<div class="wcfm-container">
				...
				<thead>
					<tr>
					  <th class="item-thumb no_mob" data-sort="string-ins"></th>
						<th class="item sortable" data-sort="string-ins"><?php _e( 'Item', $WCFM->text_domain ); ?></th>
						<?php do_action( 'woocommerce_admin_order_item_headers', $order ); ?>
						<th class="item_cost sortable no_mob" data-sort="float"><?php _e( 'Cost', $WCFM->text_domain ); ?></th>
						<th class="quantity sortable no_mob" data-sort="int"><?php _e( 'Qty', $WCFM->text_domain ); ?></th>
						<?php if( $is_wcfm_order_details_line_total_head = apply_filters( 'wcfm_order_details_line_total_head', true ) ) { ?>
						  <th class="line_cost sortable" data-sort="float"><?php _e( 'Total', $WCFM->text_domain ); ?></th>
						<?php } ?>
							...
							<?php
								if ( empty( $legacy_order ) && ! empty( $order_taxes ) ) :
									foreach ( $order_taxes as $tax_id => $tax_item ) :
										...
										$tax_class_name = isset( $classes_options[ $tax_class ] ) ? $classes_options[ $tax_class ] : __( 'Tax', $WCFM->text_domain );
										$column_label   = ! empty( $tax_item['label'] ) ? $tax_item['label'] : __( 'Tax', $WCFM->text_domain );
				  ...
				  $line_items = apply_filters( 'wcfm_valid_line_items', $line_items, $order->get_id() );
					foreach ( $line_items as $item_id => $item ) {
						...
						<tr class="item <?php echo apply_filters( 'woocommerce_admin_html_order_item_class', ( ! empty( $class ) ? $class : '' ), $item, $order ); ?>" data-order_item_id="<?php echo $item_id; ?>">
							...
							<td class="name" data-sort-value="<?php echo esc_attr( $item->get_name() ); ?>">
								<?php
									...
									if ( $_product && $_product->get_sku() ) {
										echo '<div class="wc-order-item-sku"><strong>' . __( 'SKU:', $WCFM->text_domain ) . '</strong> ' . esc_html( $_product->get_sku() ) . '</div>';
									}
						
									if ( ! empty( $item->get_variation_id() ) ) {
										echo '<div class="wc-order-item-variation"><strong>' . __( 'Variation ID:', $WCFM->text_domain ) . '</strong> ';
										if ( ! empty( $item->get_variation_id() ) && 'product_variation' === get_post_type( $item->get_variation_id() ) ) {
											...
											echo esc_html( $item->get_variation_id() ) . ' (' . __( 'No longer exists', $WCFM->text_domain ) . ')';
										}
										echo '</div>';
									}
								?>
							...
							<td class="quantity no_mob" width="1%">
								<div class="view">
									<?php
				...
				<?php
					...
					foreach ( $line_items_shipping as $item_id => $item ) {
						...
						<tr class="shipping <?php echo ( ! empty( $class ) ) ? $class : ''; ?>" data-order_item_id="<?php echo $item_id; ?>">
							<td class="thumb no_ipad no_mob"><span class="wcicon-truck-2"></span></td>
						
							<td class="name">
								<div class="view">
									<?php echo ! empty( $item->get_name() ) ? wc_clean( $item->get_name() ) : __( 'Shipping', $WCFM->text_domain ); ?>
								</div>
				...
				<?php
					foreach ( $line_items_fee as $item_id => $item ) {
						...
						<tr class="fee <?php echo ( ! empty( $class ) ) ? $class : ''; ?>" data-order_item_id="<?php echo $item_id; ?>">
							<td class="thumb no_ipad no_mob"><span class="wcicon-plus"></span></td>
						
							<td class="name">
								<div class="view">
									<?php echo ! empty( $item->get_name() ) ? esc_html( $item->get_name() ) : __( 'Fee', $WCFM->text_domain ); ?>
								</div>
							</td>
			...
			<div class="wc-order-data-row wc-order-totals-items wc-order-items-editable">
			  <?php if( $is_wcfm_order_details_coupon_line_item = apply_filters( 'wcfm_order_details_coupon_line_item', true ) ) { ?>
						...
						if ( $coupons ) {
							...
							<div class="wc-used-coupons">
								<ul class="wc_coupon_list"><?php
									echo '<li><strong>' . __( 'Coupon(s) Used', $WCFM->text_domain ) . '</strong></li>';
				...
				<table class="wc-order-totals">
				  <?php if( $is_wcfm_order_details_coupon_line_item = apply_filters( 'wcfm_order_details_coupon_line_item', true ) ) { ?>
						<tr>
							<td class="label"><span class="img_tip" data-tip="<?php _e( 'This is the total discount. Discounts are defined per line item.', $WCFM->text_domain ) ; ?>"></span> <?php _e( 'Discount', $WCFM->text_domain ); ?>:</td>
							<td width="1%"></td>
							<td class="total">
					...
					<?php if( $is_wcfm_order_details_shipping_line_item = apply_filters( 'wcfm_order_details_shipping_line_item', true ) ) { ?>
						<tr>
							<td class="label"><span class="img_tip" data-tip="<?php _e( 'This is the shipping and handling total costs for the order.', $WCFM->text_domain ) ; ?>"></span> <?php _e( 'Shipping', $WCFM->text_domain ); ?>:</td>
							<td width="1%"></td>
							<td class="total"><?php
					...
					<tr>
						<td class="label"><?php _e( 'Order Total', $WCFM->text_domain ); ?>:</td>
						<td>
					...
					<?php if( $is_wcfm_order_details_refund_line_item = apply_filters( 'wcfm_order_details_refund_line_item', true ) ) { ?>
						<?php if ( $order->get_total_refunded() ) : ?>
							<tr>
								<td class="label refunded-total"><?php _e( 'Refunded', $WCFM->text_domain ); ?>:</td>
								<td width="1%"></td>
