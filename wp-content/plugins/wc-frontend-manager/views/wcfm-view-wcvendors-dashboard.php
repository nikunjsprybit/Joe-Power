...
$sql = "SELECT SUM( commission.total_due ) FROM {$wpdb->prefix}pv_commission AS commission";
$sql .= " WHERE 1=1";
$sql .= " AND commission.vendor_id = %d";
$sql .= " AND MONTH( commission.time ) = MONTH( NOW() )";

$earned = $wpdb->get_var( $wpdb->prepare( $sql, $user_id ) );
if( !$earned ) $earned = 0;
...
$commission = $wpdb->get_var( $wpdb->prepare( $sql, $user_id ) );
