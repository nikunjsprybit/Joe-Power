...
class WCFM_Orders_WCPVendors_Controller {
	...
	public function processing() {
		...
		$user_id = WC_Product_Vendors_Utils::get_logged_in_vendor();
		...
		$wcfm_orders_json = '{
														...
														"recordsFiltered": ' . count($the_order_summary) . ',
														"data": ';
		...
		if ( !empty( $the_order_summary ) ) {
			...
			foreach ( $the_order_summary as $order ) {
				...
				$wcfm_orders_json_arr[$index][] = '<a href="#" class="show_order_items">' . sprintf( _n( '%d item', '%d items', $quantity, $WCFM->text_domain ), $quantity ) . '</a>' . $the_order_item_details;
				...
				// Action
				$wcfm_orders_json_arr[$index][] =  apply_filters ( 'wcpvendors_orders_actions', '<a class="wcfm-action-icon" href="' . get_wcfm_view_order_url($the_order->id, $the_order) . '"><span class="fa fa-eye text_tip" data-tip="' . esc_attr__( 'View Details', $WCFM->text_domain ) . '"></span></a>', $user_id, $the_order, $order );
