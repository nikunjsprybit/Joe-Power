		...
		if(!empty($wcfm_orders_array)) {
			...
			foreach($wcfm_orders_array as $wcfm_orders_single) {
				...
				foreach ($items as $key => $item) {
					...
					if ( ! empty( $item_meta_html ) ) $order_item_details .= '<span class="img_tip custom" data-tip="' . $item_meta_html . '"></span>';
					$order_item_details .= '</td></div>';
				}
				$order_item_details .= '</div>';
				$wcfm_orders_json_arr[$index][] =  '<a href="#" class="show_order_items">' . apply_filters( 'woocommerce_admin_order_item_count', sprintf( _n( '%d item', '%d items', $the_order->get_item_count(), $WCFM->text_domain ), $the_order->get_item_count() ), $the_order ) . '</a>' . $order_item_details;
				
				// Total
				$total = '<span class="order_total">' . $the_order->get_formatted_order_total() . '</span>';

				if ( $the_order->payment_method_title ) {
					$total .= '<br /><small class="meta">' . __( 'Via', $WCFM->text_domain ) . ' ' . esc_html( $the_order->payment_method_title ) . '</small>';
				}
				$wcfm_orders_json_arr[$index][] =  $total;
				
				// Date
				if ( '0000-00-00 00:00:00' == $wcfm_orders_single->post_date ) {
					$t_time = $h_time = __( 'Unpublished', $WCFM->text_domain );
				} else {
					$t_time = get_the_time( __( 'Y/m/d g:i:s A', $WCFM->text_domain ), $wcfm_orders_single );
					$h_time = get_the_time( __( 'Y/m/d', $WCFM->text_domain ), $wcfm_orders_single );
				}
				...
				// Action
				$wcfm_orders_json_arr[$index][] =  apply_filters ( 'wcfm_orders_actions', '<a class="wcfm-action-icon" href="' . get_wcfm_view_order_url($wcfm_orders_single->ID, $the_order) . '"><span class="fa fa-eye text_tip" data-tip="' . esc_attr__( 'View Details', $WCFM->text_domain ) . '"></span></a>', $wcfm_orders_single, $the_order );
