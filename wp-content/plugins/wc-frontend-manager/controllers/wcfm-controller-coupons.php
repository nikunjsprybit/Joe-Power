	...
	public function processing() {
		...
		if(!empty($wcfm_coupons_array)) {
			...
			foreach($wcfm_coupons_array as $wcfm_coupons_single) {
				...
				// Code
				// href="' . get_wcfm_coupons_manage_url($wcfm_coupons_single->ID) . '"
				if( $wcfm_coupons_single->post_status != 'publish' ) {
					$wcfm_coupons_json_arr[$index][] =  '<a  href="'.wpestate_my_reservations_link().'/?action=edit&id='.$wcfm_coupons_single->ID.'"  class="wcfm_coupon_title">' . $wcfm_coupons_single->post_title . '</a>' . ' -- ' . ucfirst( $wcfm_coupons_single->post_status );
				} elseif( current_user_can( 'edit_published_shop_coupons' ) ) {
					$wcfm_coupons_json_arr[$index][] =  '<a href="'.wpestate_my_reservations_link().'/?action=edit&id='.$wcfm_coupons_single->ID.'" class="wcfm_coupon_title">' . $wcfm_coupons_single->post_title . '</a>';
				} else {
				...
				// Expire Date
				$wcfm_coupons_json_arr[$index][] = $wc_coupon->get_date_expires() ? $wc_coupon->get_date_expires()->date( 'Y-m-d' ) : '&ndash;';

				// Action
				$actions = '';
				if( $wcfm_coupons_single->post_status == 'publish' ) {
				  $actions .= ( current_user_can( 'edit_published_shop_coupons' ) || in_array( 'dc_vendor', (array) $user->roles ) ) ? '<a class="wcfm-action-icon" href="'.wpestate_my_reservations_link().'/?action=edit&id='.$wcfm_coupons_single->ID.'"><span class="fa fa-edit text_tip" data-tip="' . esc_attr__( 'Edit', $WCFM->text_domain ) . '"></span></a>' : '';
				} else {
					$actions .= ( current_user_can( 'edit_shop_coupons' ) || in_array( 'dc_vendor', (array) $user->roles )) ? '<a class="wcfm-action-icon" href="'.wpestate_my_reservations_link().'/?action=edit&id='.$wcfm_coupons_single->ID.'"><span class="fa fa-edit text_tip" data-tip="' . esc_attr__( 'Edit', $WCFM->text_domain ) . '"></span></a>' : '';
				}
