	  ...
	  if(isset($wcfm_coupon_manager_form_data['title']) && !empty($wcfm_coupon_manager_form_data['title'])) {
	  	...
	  	// Creating new coupon
			$new_coupon = array(
				...
				'post_excerpt' => $wcfm_coupon_manager_form_data['description'],
			...
			if(!is_wp_error($new_coupon_id)) {
				...
				if(!$has_error) {
					if( get_post_status( $new_coupon_id ) == 'draft' ) {
						...
						if(!$has_error) echo '{"status": true, "message": "' . $wcfm_coupon_messages['coupon_published'] . '", "redirect": "' . wpestate_my_reservations_link() . '"}';
					}
				}
				die;
			}
